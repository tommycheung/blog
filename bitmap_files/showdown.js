 ...
 * 
 *
 * - Customize it to support SyntaxHighlighter and others
 * - fix a new line bug for code type detection
 * 
 */
...
this.makeReadHtml = function (text) {
  //
...
//
  ...
  // 7N: remove all inject css
  text = text.replace(/<link[\s\S\w\W]*\/>/g,"");
  ...
  text = text.replace(/https?\:\/\/[^"\s\<\>]*[^.,;'">\:\s\<\>\)\]\!]/g, function (wholeMatch,matchIndex) {
    ...
    if ((left.lastIndexOf("<code") > left.lastIndexOf("</code>")) &&
        ((right.indexOf("</code>") < right.indexOf("<code")) || (right.indexOf("</code>") != -1 && right.indexOf("<code") == -1))) {
        return wholeMatch;
    }
    
    if ((left.lastIndexOf("<pre") > left.lastIndexOf("</pre>")) &&
        ((right.indexOf("</pre>") < right.indexOf("<pre")) || (right.indexOf("</pre>") != -1 && right.indexOf("<pre") == -1))) {
        return wholeMatch;
    }
    
    href = wholeMatch;
    var urlreg = /(https?|ftp|mms):\/\/((([A-z0-9]+[_\-]?[A-z0-9]+\.)*[A-z0-9]+\-?[A-z0-9]+\.[A-z]{2,})|(([0-9]{1,3}\.){3}[0-9]{1,3}))(\/.*)*\/?/;
    return "<a href='" + (urlreg.test(href)?href:(href)) + "'>" + wholeMatch + "</a>";
  });
...
//
  ...
  text = text.replace(/https?\:\/\/[^"\s\<\>]*[^.,;'">\:\s\<\>\)\]\!]/g, function (wholeMatch,matchIndex) {
    ...
    href = wholeMatch;
    var urlreg = /(https?|ftp|mms):\/\/((([A-z0-9]+[_\-]?[A-z0-9]+\.)*[A-z0-9]+\-?[A-z0-9]+\.[A-z]{2,})|(([0-9]{1,3}\.){3}[0-9]{1,3}))(\/.*)*\/?/;
    //return "<a href='" + (urlreg.test(href)?href:("http://atatech.org"+href)) + "'>" + wholeMatch + "</a>";
    return "<a href='" + (urlreg.test(href)?href:(href)) + "'>" + wholeMatch + "</a>";
  });
  text = text.replace(/[a-z0-9_\-+=.]+@[a-z0-9\-]+(\.[a-z0-9-]+)+/ig, function(wholeMatch){return "<a href='mailto:" + wholeMatch + "'>" + wholeMatch + "</a>";});
...
var _RunReadBlockGamut = function(text) {
//
...
var _DoCodeBlocksBefore = function (text) {
  ...
  text = text.replace(/```(\w*\n*)((.|\n)*?)```/g,
    function (wholeMatch, prepadding, codeblock) {
      ...
      return '```' + prepadding + _EncodeCode(codeblock).replace(/^ +| +$/g, '') + '```';
    }
  );
  return text;
}

var _DoReadCodeBlocks = function(text) {
//
...
//
  ...
  text = text.replace(/```(\w+)?\n*((.|\n)*?)```/g,
    function (wholeMatch, lang, codeblock) {
      ...
      var brush = "brush:" + brushValue + ";" ;

      // codeblock = _EncodeCode(codeblock).replace(/^\s+|\s+$/g,"");
      codeblock = "<pre class=\"prettyprint linenums " + lang +  " " + brush +  "\">" + codeblock + "</pre>";
  ...
  */
  ...
  text = text.replace(/(?:\n\n|^)((?:(?:[ ]{4}|\t).*\n+)+)(\n*[ ]{0,3}[^ \t\n]|(?=~0))/g,
    function (wholeMatch,m1,m2) {
      ...
      codeblock = "<pre class=\"prettyprint linenums brush:php\">" + codeblock + "\n</pre>";
...
//
  ...
  text = text.replace(/```(\w+)?\n*((.|\n)*?)```/g,
    function (wholeMatch, lang, codeblock) {
      ...
      // codeblock = _EncodeCode(codeblock).replace(/^\s+|\s+$/g,"");
      codeblock = "<pre class=\"prettyprint linenums " + lang +  " " + brush +  "\"><code>" + codeblock + "</code></pre>";
      return hashBlock(codeblock);
    }
  );
  ...
  */
  ...
  text = text.replace(/(?:\n\n|^)((?:(?:[ ]{4}|\t).*\n+)+)(\n*[ ]{0,3}[^ \t\n]|(?=~0))/g,
    function (wholeMatch,m1,m2) {
      ...
      codeblock = "<pre class=\"prettyprint linenums brush:php\"><code>" + codeblock + "\n</code></pre>";
      return hashBlock(codeblock) + nextChar;
    }
  );
